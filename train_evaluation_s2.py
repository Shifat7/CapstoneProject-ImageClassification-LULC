...
from Transformer_SSL.models.swin_transformer import (
    ...
    DoubleSwinTransformerSegmentationS2,
...
model_name_map = {
    ...
    "DoubleSwinTransformerSegmentationS2": "swin-t",
    "MobyDownstream": "moby",
...
# train set
parser.add_argument(
    "--train_dir", default="splits", type=str
)
...
# test set
parser.add_argument(
    "--val_dir", default="splits", type=str
)
...
if model_name == "baseline" or model_name == "swin-baseline":
    ...
    input_channels = config.s2_input_channels
    ...
    #s1_backbone = build_model(swin_conf.model_config) *** NOT USING S1
    swin_conf.model_config.MODEL.SWIN.IN_CHANS = 13
    s2_backbone = build_model(swin_conf.model_config)
    checkpoint = torch.load(
        config.checkpoint,
	map_location=torch.device('cpu') # added to force use CPU!!!!!!!!!!!!!!!!!
    ...
    if target_name == "pixel-classification":
        model = DoubleSwinTransformerSegmentationS2(
            s2_backbone, out_dim=8, device=device # removed s1_backbone, out_dim = num classes
        )
    else:
        model = DoubleSwinTransformerDownstream(
            s2_backbone, out_dim=8, device=device # removed s1_backbone
        )
...
for epoch in range(config.epochs):
    ...
    #if epoch % 200 == 0: ADAPTED TO SHORTEN PROCESS FOR TESTING PURPOSES
    if epoch % 2 == 0:
        if epoch == 0:
            continue
        ...
        torch.save(model.state_dict(), save_weights_path) # code from Jupyter Notebook version

        '''save_checkpoint_single_model( # commented out as don't know how to use sub-keys
