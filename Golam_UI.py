...
class WorkerThread(QThread):
    ...
    def run(self):
        for progress_value in range(0, 101):
            ...
            self.msleep(10)  # Simulate a time-consuming operation
...
class DesktopUI(QMainWindow):
    ...
    def segment_patches(self):
        ...
        total_patches = len(selected_patch_names)
        for index, patch_name in enumerate(selected_patch_names):
            
            # Segment the patch using your model's logic
            main([patch_name])  # Process one patch at a time
            
            # Update the progress bar
            progress_value = int((index + 1) / total_patches * 100)
    ...
    def stop_segmentation(self,open_map_dialog):
        print("Segmentation stopped!")
        open_map_dialog(self).close
