...
class SwinTransformer(nn.Module):
    ...
    def flops(self):
        ...
        for i, layer in enumerate(self.layers):
            flops += layer.flops()
        flops += (
            self.num_features * self.patches_resolution[0] * self.patches_resolution[1] // (2**self.num_layers)
        )
        flops += self.num_features * self.num_classes
        return flops


class DoubleSwinTransformer(nn.Module):
    def __init__(self, encoder1, encoder2):
        super(DoubleSwinTransformer, self).__init__()
...
class SwinTransformerDecoder(nn.Module):
    def __init__(
        ...
        **kwargs,):
        ...
        self.mlp_ratio = self.backbone.mlp_ratio
        self.patches_resolution = self.backbone.patches_resolution
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                ...
                layer_up = BasicLayer_up(
                    dim=int(
                        self.embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                    input_resolution=(
                        self.patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
                        self.patches_resolution[1] // (2 ** (self.num_layers - 1 - i_layer))),
        ...
        self.up = FinalPatchExpand_X4(
            input_resolution=(img_size // patch_size, img_size // patch_size), dim_scale=4, dim=2 * self.embed_dim)
        self.output = nn.Conv2d(in_channels=2 * self.embed_dim, out_channels=self.out_dim, kernel_size=1, bias=False)
...
class SharedDSwin(torch.nn.Module):
    """multimodal double t-swin transformer with two encoders (stage/swin-block 1)
    and a shared head (swin-stage/blocks 2,3,4)"""

    def __init__(self, s1_enc, s2_enc, shared_backbone):
        super(SharedDSwin, self).__init__()
        self.s1_enc = s1_enc
        self.s2_enc = s2_enc
        self.shared_backbone = shared_backbone

    def forward(self, images):
        """get features for s1 and s2 images (no connection between the two necessary)
        after shared backbone part"""
        s1 = images["s1"]
        s2 = images["s2"]
        features = self.forward_encoders(s1, s2)
        z1 = self.shared_backbone.forward_features(features["s1"])
        z2 = self.shared_backbone.forward_features(features["s2"])
    ...
    def forward_encoders(self, s1, s2):
        """get feature representations for s1 and s2 images after non-shared backbones"""
        z1 = self.s1_enc(s1)
        z2 = self.s2_enc(s2)
...
class DownstreamSharedDSwin(nn.Module):
    def __init__(self, shared_model, out_dim, freeze_layers=True):
        super(DownstreamSharedDSwin, self).__init__()
        self.backbone = shared_model
        self.fc = torch.nn.Linear(768 * 2, out_dim, bias=True)
    ...
    def forward(self, sample):
        ...
        # fuse representations of s1 and s2 images
        z = torch.cat([sample["s1"], sample["s2"]], dim=1)
